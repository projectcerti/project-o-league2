...
export default function AdminPanel() {
  ...
  async function savePointOverride() {
    if (overridePts === '' || !overrideModal) return
    const pts = parseInt(overridePts)
    if (isNaN(pts) || pts < 0 || pts > 11) return
    setSavingOverride(true)
    try {
      // Upsert weekly_submissions with admin override
      const { data: existing } = await supabase
        .from('weekly_submissions')
        .select('id')
        .eq('user_id', overrideModal.userId)
        .eq('week_number', overrideModal.weekNum)
        .maybeSingle()

      if (existing) {
        await supabase.from('weekly_submissions')
          .update({ admin_override_points: pts })
          .eq('id', existing.id)
      } else {
        await supabase.from('weekly_submissions').insert({
          user_id: overrideModal.userId,
          week_number: overrideModal.weekNum,
          admin_override_points: pts,
          calculated_points: 0,
          workouts: 0,
          recovery_sessions: 0,
          social_sessions: 0,
      ...
      const reasonText = overrideReason.trim()
        ? `Your points for Week ${overrideModal.weekNum} were set to ${pts}/11 by an admin. Reason: ${overrideReason.trim()}`
        : `Your points for Week ${overrideModal.weekNum} were set to ${pts}/11 by an admin.`
      ...
      console.error('Point override error:', err)
    }
