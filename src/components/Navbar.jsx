...
const Icons = {
  ...
  bell:     (a) => <svg width="21" height="21" viewBox="0 0 24 24" fill={a?'currentColor':'none'} stroke="currentColor" strokeWidth="1.8"><path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 01-3.46 0"/></svg>,
...
export default function Navbar() {
  ...
  const tabs = isOwner
    ? [baseTabs[0], baseTabs[1], baseTabs[2], { to: '/admin', icon: 'admin', label: 'Admin' }, baseTabs[4]]
    : baseTabs
  ...
  return (
    <>
      ...
      <nav className="hidden md:flex sticky top-0 z-50 bg-bg/95 backdrop-blur border-b border-border">
        <div className="max-w-5xl mx-auto px-5 w-full h-13 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {/* Notification bell */}
            <Link to="/notifications" className="relative p-1.5 text-muted hover:text-white transition-colors">
              {Icons.bell(false)}
              {unreadCount > 0 && (
                <span className="absolute -top-0.5 -right-0.5 bg-lime text-bg text-xs font-kanit font-bold w-4 h-4 rounded-full flex items-center justify-center">
      ...
      <div className="md:hidden sticky top-0 z-50 bg-bg/98 backdrop-blur border-b border-border px-5 h-11 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          {/* Notification bell */}
          <Link to="/notifications" className="relative p-1 text-muted hover:text-white transition-colors">
            {Icons.bell(false)}
            {unreadCount > 0 && (
              <span className="absolute -top-1 -right-1 bg-lime text-bg text-xs font-kanit font-bold w-4 h-4 rounded-full flex items-center justify-center leading-none">
                {unreadCount > 9 ? '9+' : unreadCount}
              </span>
            )}
          </Link>
